//
//  NFFoodListTableViewController.swift
//  NeuFood
//
...
class NFFoodListTableViewController: UITableViewController {
    var shopId: NSNumber!
    ...
    var collectionBarBtnItem: UIBarButtonItem!
    ...
    func collectionShop(){
        ...
        NFNetworkMngTool.sharedNetworkingTool().NFNetwork_CollectShopWithParameters(parameters2, view: self.view) {(success) in
            if success == "1"{
                self.isCollected()
            }else{
    ...
    func isCollected(){
        let parameters: NSDictionary = ["user_id":userId,"shop_food_id":self.shopId,"flag":0]
        as NSDictionary
        NFNetworkMngTool.sharedNetworkingTool().NFNetwork_isCollectedWithParameters(parameters, view: self.view) {
            (isCollect) in
            self.isCollect = isCollect
            if self.isCollect == "1" {
                self.rightBtn.setImage(UIImage.init(named: "Favor.png"), for: UIControlState.normal)
            }else if self.isCollect == "0" {
                self.rightBtn.setImage(UIImage.init(named: "cry.png"), for: UIControlState.normal)
            }
        }
    }
